/**
 * APP CORE ENGINE
 * TAGI TERÜLET (Mobile First & Senior Friendly)
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.toggleWatched = function(videoId) {
    ...
    if (btn) {
      ...
      btn.innerHTML = nowWatched
        ? '<i data-lucide="check-circle" style="width:18px;height:18px;"></i> <span>Megtekintve</span>'
        : '<i data-lucide="circle" style="width:18px;height:18px;"></i> <span>Megtekintettként megjelölve</span>';
    ...
    if (APP_DATA.config && APP_DATA.config.showFloatingHelp) {
      if (currentHash === "#contato") {
        ...
        floatingHelp.innerHTML = `
          <a href="${mailHref}" class="floating-help-btn" title="Segítség & Támogatás">
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Tagi Terület";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Akusztikus Gitár Mastery"}. Minden jog fenntartva.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Üdvözlünk, Tag!</h1><p>Mihez szeretnél hozzáférni ma?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Könyvek &amp; PDF anyagok</div>
              </div>
            </a>
            ...
            <a href="#videos" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Videók</div>
                 <div class="home-block-subtitle">Videóleckék &amp; Útmutatók</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Kapcsolat</div>
                 <div class="home-block-subtitle">Segítség &amp; Támogatás</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    let featuredHTML = mainBooks.map(bk => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'KÜLÖNLEGES'}</span>
               <span class="premium-format">PDF &bull; Letölthető Dokumentum</span>
            </div>
              ...
              ${featuresHTML}
               ...
               <div style="display: flex; flex-direction: column; gap: 0.75rem; width: 100%; margin-top: 1.5rem;">
                  <a href="${bk.downloadUrl}" target="_blank" class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: var(--primary); color: #FFF;">
                     <i data-lucide="book-open"></i> Olvasás most
                  </a>
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">További Források</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Forrás'}</span>
            </div>
            <div class="compact-book-actions">
              <a href="${bk.downloadUrl}" target="_blank" class="compact-action-btn btn-read" title="Olvasás most">
                <i data-lucide="book-open"></i>
              </a>
              <a href="${bk.downloadUrl}" download class="compact-action-btn btn-download" title="PDF Letöltése">
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Saját Anyagaid</h1><p>Kattints az alábbi gyűjteményekre a könyveid megtekintéséhez és letöltéséhez.</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/gui_IMG1_hu.png" alt="Akusztikus Gitár Mastery" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>

          <div class="list-container">
            ${featuredHTML || '<p>Jelenleg nincs elérhető anyag.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderOutrosProdutos() {
    ...
    let prodsHTML = prods.map(prod => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${prod.badgeColor || 'var(--primary)'}">${prod.badgeText || 'KÜLÖNLEGES'}</span>
               <span class="premium-format">Online Hozzáférés</span>
            </div>
           ...
           <div class="premium-info">
              ...
              <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                 <i data-lucide="external-link"></i> ${prod.buttonText || 'Tudj meg többet'}
              </a>
           </div>
         </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Egyéb Programok</h1><p>Fedezz fel további programokat és anyagokat.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Új frissítések hamarosan!</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Ügyfélszolgálat</h1><p>Az Ön elégedettsége a mi prioritásunk.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Másold ki az alábbi e-mail címet, és küldd el nekünk a kérdésedet. Támogatási csapatunk a lehető leghamarabb válaszol.
             </p>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Videóleckék</h1><p>Nézd meg a gitárakkord útmutatókat kezdőknek — nincs beszéd, csak gitárhang.</p></div></div>
            ...
            <a href="#videos" onclick=\"window.selectVideo('${vid.id}'); return false;\" style="display: flex; align-items: center; gap: 12px; padding: 16px; text-decoration: none; color: inherit;">
              ...
              <div style="display:flex; flex-direction:column; justify-content:center; flex:1; min-width:0;">
                 ...
                 <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                   <p style="margin:0; font-size:0.82rem; color:var(--text-muted);">${vid.duration || 'Teljes lecke'}</p>
                   ${`<span id="watched-badge-${vid.id}" style="display:${typeof localStorage !== 'undefined' && localStorage.getItem('watched_'+vid.id)==='1'?'inline-flex':'none'}; align-items:center; gap:3px; font-size:0.72rem; font-weight:700; color:var(--primary); background:var(--primary-light); padding:2px 8px; border-radius:20px;"><i data-lucide="check" style="width:10px;height:10px;"></i> Megtekintve</span>`}
                 </div>
              </div>
              ${isPlaying
              ...
              <div class="play-item-body" style="padding: 0 16px 16px 16px; animation: slideDown 0.3s ease;">
                 ...
                 <div style="background:var(--bg-card); border:1px solid var(--border-light); border-radius:0 0 12px 12px; padding: 10px 14px; display:flex; flex-direction:column; gap:10px;">
                   ...
                   <div style="display:flex; justify-content:center; align-items:center; gap:10px;">
                     <button onclick="window.restartYtVideo()" title="Újraindítás" style="background:var(--bg-body); color:var(--text-dark); border:1px solid var(--border-light); border-radius:50%; width:40px; height:40px; display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0;">
                     ...
                     <button onclick="window.toggleCustomFullscreen('video-container-${vid.id}')" title="Teljes képernyő" style="background:var(--bg-body); color:var(--text-dark); border:1px solid var(--border-light); border-radius:50%; width:40px; height:40px; display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0;">
                   ...
                   <div style="display:flex; align-items:center; gap:6px; justify-content:center; flex-wrap:wrap;">
                     <span style="font-size:0.75rem; font-weight:600; color:var(--text-muted); margin-right:4px;">SEBESSÉG</span>
                   ...
                   <div style="display:flex; justify-content:center;">
                     <button id="watched-btn-${vid.id}" onclick="window.toggleWatched('${vid.id}')" class="${localStorage.getItem('watched_${vid.id}')==='1'?'watched-active':''}" style="display:flex; align-items:center; gap:6px; padding:8px 20px; border-radius:30px; border:1.5px solid ${localStorage.getItem('watched_${vid.id}')==='1'?'var(--primary)':'var(--border-light)'}; background:${localStorage.getItem('watched_${vid.id}')==='1'?'var(--primary-light)':'transparent'}; color:${localStorage.getItem('watched_${vid.id}')==='1'?'var(--primary)':'var(--text-muted)'}; cursor:pointer; font-size:0.85rem; font-weight:600; transition:all 0.2s;">
                       ...
                       <span>${localStorage.getItem('watched_${vid.id}')==='1'?'Megtekintve':'Megtekintettként megjelölve'}</span>
                     </button>
                   </div>
