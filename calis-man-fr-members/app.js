/**
 * APP CORE ENGINE
 * ESPACE MEMBRE (Mobile First & Senior Friendly)
 * Calisthénie Militaire pour Hommes — Français
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Calisthénie Militaire pour Hommes";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Calisthénie Militaire pour Hommes"}. Tous droits réservés.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Bienvenue, cher membre !</h1><p>À quels programmes d'entraînement souhaitez-vous accéder aujourd'hui ?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Livres et guides complets en PDF</div>
              </div>
            </a>
            ...
            <a href="#videos" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Vidéos d'entraînement et techniques</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Aide et service client</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">Programmes complémentaires</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Document'}</span>
            </div>
            <div class="compact-book-actions">
              ...
              <a href="${bk.downloadUrl}" download class="compact-action-btn btn-download" title="Télécharger PDF">
    ...
    const heroCoverImg = (APP_DATA.books && APP_DATA.books[0] && APP_DATA.books[0].coverImage) ? APP_DATA.books[0].coverImage : "assets/covers/man_IMG1_fr.png";
    const heroCoverAlt = (APP_DATA.config && APP_DATA.config.brandName) ? APP_DATA.config.brandName : "Calisthénie Militaire pour Hommes";

    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Vos Programmes</h1><p>Cliquez sur les collections ci-dessous pour lire et télécharger vos guides.</p></div></div>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Vidéos d'Entraînement</h1><p>Exercices militaires au poids du corps et technique précise pour hommes 40+.</p></div></div>
        ...
        <div style="background-color: rgba(16, 185, 129, 0.1); color: var(--primary); border: 1px solid var(--border-light); padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; display: flex; align-items: center; gap: 10px; font-weight: 500; font-size: 0.95rem;" class="glass-panel">
           ...
           <span>Cliquez sur un exercice pour lancer la vidéo de démonstration</span>
        </div>
      ...
      ${isPlaying ? `
              <div class="play-item-body" style="padding: 0 16px 16px 16px; animation: slideDown 0.3s ease;">
                 ...
                 <div style="margin-top: 10px; padding: 10px 14px; background: var(--bg-card); border-radius: 8px; border: 1px solid var(--border-light); font-size: 0.88rem; color: var(--text-muted); display: flex; align-items: center; gap: 8px;">
                     ...
                     <span><strong>Consigne :</strong> ${vid.obs}</span>
                 </div>
                 ` : ''}
              </div>
            ` : ''}
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Programmes recommandés</h1><p>Découvrez d'autres guides et formations d'exception.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Aucun autre produit n\'est disponible pour le moment.</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Service Client & Support</h1><p>Votre satisfaction est notre priorité absolue.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Envoyez-nous un message</h3>
