...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Проста без глютэну"}. Усе правы абаронены.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Сардэчна запрашаем!</h1><p>Да якога кантэнту вы хочаце атрымаць доступ сёння?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Электронныя кнігі і PDF-матэрыялы</div>
              </div>
            </a>
  ...
  function renderLivros() {
    const booksHTML = APP_DATA.books.map(bk => {
      ...
      <div class="premium-book-card" style="margin-bottom: 2rem;">
           <div class="premium-badge-wrapper">
              ...
              <span class="premium-format">PDF · Чытаць зараз</span>
           </div>
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel" style="margin-bottom: 1.5rem;"><div class="hero-text"><h1>Вашы матэрыялы</h1><p>Атрымайце доступ да вашых інструкцый і бонусаў ніжэй.</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/gf_IMG1_be.png" alt="Камплект Проста без глютэну" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>
          
          <div class="list-container">
            ${booksHTML || '<p>На дадзены момант няма зарэгістраваных матэрыялаў.</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Іншыя праграмы</h1><p>Адкрыйце для сябе матэрыялы для паляпшэння здароўя.</p></div></div>
          
          
          <div class="list-container">
            ${prodsHTML || '<p>Хутка будуць новыя паведамленні!</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Падтрымка карыстальнікаў</h1><p>Ваша задаволенасць — наш прыярытэт.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                Скапіруйце адрас ніжэй і адпраўце ліст з вашым пытаннем. Наша каманда адкажа як мага хутчэй.
             </p>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Вашы ўрокі</h1><p>Сачыце за сваімі анлайн-практыкаваннямі</p></div></div>
      ...
      <div class="card-bloco play-item glass-panel ${isPlaying ? 'active-play' : ''}" style="margin-bottom:16px; display:flex; flex-direction:column; padding:0; overflow:hidden;" data-video-id="${vid.id}">
            ...
            <a href="javascript:void(0)" class="play-item-header" style="display:flex; padding: 16px; text-decoration:none; color:inherit;">
              <div style="display:flex; flex-direction:column; justify-content:center; flex:1">
                 ...
                 <p style="margin:0; font-size:0.9rem; color:${isPlaying ? 'var(--text-dark)' : 'var(--text-muted)'}">${vid.duration || 'Поўны ўрок'}</p>
              </div>
              ${isPlaying
