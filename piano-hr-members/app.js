/**
 * APP CORE ENGINE
 * KORISNIČKO PODRUČJE (Mobile First & Senior Friendly)
 * Potpuna Kolekcija za Klavir — Hrvatski
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleFsUpdate() {
      ...
      document.querySelectorAll('.video-custom-controls').forEach(bar => {
          ...
          if (btn) {
              btn.innerHTML = isFs 
                  ? `<i data-lucide="minimize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Smanji</span>`
                  : `<i data-lucide="maximize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Cijeli zaslon</span>`;
          }
      });
      renderIcons();
  }
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Potpuna Kolekcija za Klavir";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#cbd5e1; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Potpuna Kolekcija za Klavir"}. Sva prava pridržana.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Dobrodošli, dragi člane!</h1><p>Koje materijale želite danas otvoriti?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Knjige i praktični vodiči u PDF formatu</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Podrška</div>
                 <div class="home-block-subtitle">Pomoć i korisnička podrška</div>
              </div>
            </a>
  ...
  function renderLivros() {
               ...
               ${featuresHTML}
                ...
                <div style="display: flex; flex-direction: column; gap: 0.75rem; width: 100%; margin-top: 1.5rem;">
                   <a href="${bk.downloadUrl}" target="_blank" class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: #6b1d2f; color: #FFF;">
                      <i data-lucide="book-open"></i> Čitaj odmah
                   </a>
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">Dodatni materijali</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Materijal'}</span>
            </div>
            <div class="compact-book-actions">
              <a href="${bk.downloadUrl}" target="_blank" class="compact-action-btn btn-read" title="Čitaj odmah">
                <i data-lucide="book-open"></i>
              </a>
    ...
    const heroCoverImg = (APP_DATA.books && APP_DATA.books[0] && APP_DATA.books[0].coverImage) ? APP_DATA.books[0].coverImage : "assets/covers/piano_IMG1_hr.png";
    const heroCoverAlt = (APP_DATA.config && APP_DATA.config.brandName) ? APP_DATA.config.brandName : "Potpuna Kolekcija za Klavir";

    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Vaši materijali</h1><p>Kliknite na zbirke u nastavku za čitanje i preuzimanje svojih knjiga.</p></div></div>
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Drugi programi</h1><p>Otkrijte više preporučenih vodiča i tečajeva.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Trenutno nema drugih dostupnih proizvoda.</p>'}
          </div>
        </div>
      `;
  }

  // PRIKAZ: KONTAKT / PODRŠKA
  function renderContato() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Korisnička podrška</h1><p>Vaše zadovoljstvo naš je najveći prioritet.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Pošaljite nam poruku</h3>
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Kopirajte e-mail adresu u nastavku i pošaljite nam svoj upit. Naš tim podrške odgovorit će vam u najkraćem mogućem roku.
             </p>
