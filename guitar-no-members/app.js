/**
 * APP CORE ENGINE
 * MEDLEMSOMRÅDE (Mobile First & Senior Friendly)
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.toggleWatched = function(videoId) {
    ...
    if (btn) {
      ...
      btn.innerHTML = nowWatched
        ? '<i data-lucide="check-circle" style="width:18px;height:18px;"></i> <span>Sett</span>'
        : '<i data-lucide="circle" style="width:18px;height:18px;"></i> <span>Marker som sett</span>';
    ...
    if (APP_DATA.config && APP_DATA.config.showFloatingHelp) {
      if (currentHash === "#contato") {
        ...
        floatingHelp.innerHTML = `
          <a href="${mailHref}" class="floating-help-btn" title="Hjelp & Support">
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Akustisk Gitar Mastery"}. Alle rettigheter forbeholdt.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Velkommen, medlem!</h1><p>Hva vil du ha tilgang til i dag?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Bøker</div>
                 <div class="home-block-subtitle">Bøker &amp; PDF-materiell</div>
              </div>
            </a>
            ...
            <a href="#videos" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Videoleksjoner &amp; Guider</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Hjelp &amp; Support</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    let featuredHTML = mainBooks.map(bk => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'SPESIELL'}</span>
               <span class="premium-format">PDF &bull; Nedlastbart Dokument</span>
            </div>
              ...
              ${featuresHTML}
               ...
               <div style="display: flex; flex-direction: column; gap: 0.75rem; width: 100%; margin-top: 1.5rem;">
                  <a href="${bk.downloadUrl}" target="_blank" class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: var(--primary); color: #FFF;">
                     <i data-lucide="book-open"></i> Les nå
                  </a>
                  <a href="${bk.downloadUrl}" download class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: transparent; color: var(--text-dark); border: 1px solid var(--border-light);">
                     <i data-lucide="download"></i> Last ned PDF
                  </a>
               </div>
           </div>
        </div>
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">Ytterligere Ressurser</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Ressurs'}</span>
            </div>
            <div class="compact-book-actions">
              <a href="${bk.downloadUrl}" target="_blank" class="compact-action-btn btn-read" title="Les nå">
                <i data-lucide="book-open"></i>
              </a>
              <a href="${bk.downloadUrl}" download class="compact-action-btn btn-download" title="Last ned PDF">
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Ditt Materiell</h1><p>Klikk på samlingene nedenfor for å se og laste ned bøkene dine.</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/gui_IMG1_no.png" alt="Akustisk Gitar Mastery" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>

          <div class="list-container">
            ${featuredHTML || '<p>Ingen materiell tilgjengelig for øyeblikket.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderOutrosProdutos() {
    ...
    let prodsHTML = prods.map(prod => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${prod.badgeColor || 'var(--primary)'}">${prod.badgeText || 'SPESIELL'}</span>
               <span class="premium-format">Online Tilgang</span>
            </div>
           ...
           <div class="premium-info">
              ...
              <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                 <i data-lucide="external-link"></i> ${prod.buttonText || 'Les mer'}
              </a>
           </div>
         </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Andre Programmer</h1><p>Oppdag flere programmer og materiell.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Nye oppdateringer kommer snart!</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Kundesupport</h1><p>Din fornøydhet er vår prioritet.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Send en melding</h3>
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Kopier e-postadressen nedenfor og send spørsmålet ditt til oss. Vårt supportteam svarer så raskt som mulig.
             </p>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Videoleksjoner</h1><p>Se gitargrep-guider for nybegynnere — ingen prating, bare gitarlyd.</p></div></div>
        ...
        <div style="background-color: rgba(16, 185, 129, 0.1); color: var(--primary); border: 1px solid var(--border-light); padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; display: flex; align-items: center; gap: 10px; font-weight: 500; font-size: 0.95rem;" class="glass-panel">
           ...
           <span>Nye leksjoner kommer snart</span>
        </div>
            ...
            <a href="#videos" onclick=\"window.selectVideo('${vid.id}'); return false;\" style="display: flex; align-items: center; gap: 12px; padding: 16px; text-decoration: none; color: inherit;">
              ...
              <div style="display:flex; flex-direction:column; justify-content:center; flex:1; min-width:0;">
                 ...
                 <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                   <p style="margin:0; font-size:0.82rem; color:var(--text-muted);">${vid.duration || 'Full leksjon'}</p>
                   ${`<span id="watched-badge-${vid.id}" style="display:${typeof localStorage !== 'undefined' && localStorage.getItem('watched_'+vid.id)==='1'?'inline-flex':'none'}; align-items:center; gap:3px; font-size:0.72rem; font-weight:700; color:var(--primary); background:var(--primary-light); padding:2px 8px; border-radius:20px;"><i data-lucide="check" style="width:10px;height:10px;"></i> Sett</span>`}
                 </div>
              </div>
              ${isPlaying
              ...
              <div class="play-item-body" style="padding: 0 16px 16px 16px; animation: slideDown 0.3s ease;">
                 ...
                 <div style="background:var(--bg-card); border:1px solid var(--border-light); border-radius:0 0 12px 12px; padding: 10px 14px; display:flex; flex-direction:column; gap:10px;">
                   ...
                   <div style="display:flex; justify-content:center; align-items:center; gap:10px;">
                     <button onclick="window.restartYtVideo()" title="Omstart" style="background:var(--bg-body); color:var(--text-dark); border:1px solid var(--border-light); border-radius:50%; width:40px; height:40px; display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0;">
                     ...
                     <button onclick="window.toggleCustomFullscreen('video-container-${vid.id}')" title="Fullskjerm" style="background:var(--bg-body); color:var(--text-dark); border:1px solid var(--border-light); border-radius:50%; width:40px; height:40px; display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0;">
                   ...
                   <div style="display:flex; align-items:center; gap:6px; justify-content:center; flex-wrap:wrap;">
                     <span style="font-size:0.75rem; font-weight:600; color:var(--text-muted); margin-right:4px;">HASTIGHET</span>
                   ...
                   <div style="display:flex; justify-content:center;">
                     <button id="watched-btn-${vid.id}" onclick="window.toggleWatched('${vid.id}')" class="${localStorage.getItem('watched_${vid.id}')==='1'?'watched-active':''}" style="display:flex; align-items:center; gap:6px; padding:8px 20px; border-radius:30px; border:1.5px solid ${localStorage.getItem('watched_${vid.id}')==='1'?'var(--primary)':'var(--border-light)'}; background:${localStorage.getItem('watched_${vid.id}')==='1'?'var(--primary-light)':'transparent'}; color:${localStorage.getItem('watched_${vid.id}')==='1'?'var(--primary)':'var(--text-muted)'}; cursor:pointer; font-size:0.85rem; font-weight:600; transition:all 0.2s;">
                       ...
                       <span>${localStorage.getItem('watched_${vid.id}')==='1'?'Sett':'Marker som sett'}</span>
                     </button>
                   </div>
