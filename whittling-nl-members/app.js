...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Ledensite";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
          &copy; 2026 ${APP_DATA.config.brandName || "Houtsnijden & Houtbewerking"}. Alle rechten voorbehouden.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Welkom, beste lid!</h1><p>Waar wilt u vandaag toegang toe krijgen?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Boeken en PDF-materialen</div>
              </div>
            </a>
    ...
    const listCardsHTML = videos.map(vid => {
      ...
      return `
        <div class="card-bloco glass-panel ${isActive ? 'active-video-item' : ''}" style="padding:1rem; cursor:pointer; display:flex; align-items:center; justify-content:space-between; gap:1rem; border:${isActive ? '1px solid var(--primary)' : '1px solid var(--border-light)'}" onclick="window.activeVideoId='${vid.id}'; location.hash='#aulas';">
           ...
           ${isWatched ? `<span style="font-size:0.75rem; background:rgba(34,197,94,0.15); color:#22c55e; padding:0.25rem 0.5rem; border-radius:6px; font-weight:600">Voltooid</span>` : ''}
        </div>
      `;
    }).join('');
    ...
    rootEl.innerHTML = `
      <div class="page-view">
        <div class="hero-card glass-panel">
           <div class="hero-text">
              <h1>Praktische Houtsnijlessen</h1>
              <p>${safeVideo ? safeVideo.title : 'Selecteer hierboven een video om te bekijken'}</p>
           </div>
        </div>
          ...
          <div class="glass-panel" style="padding:0.75rem; margin-bottom:1.5rem; border-radius:16px;">
             ...
             <div style="padding:1rem 0.5rem 0.5rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.5rem">
                ...
                <button class="download-btn" style="width:auto; padding:0.5rem 1rem; font-size:0.85rem;" onclick="const curr = localStorage.getItem('watched_${safeVideo.id}'); localStorage.setItem('watched_${safeVideo.id}', curr === '1' ? '0' : '1'); location.hash='#aulas';">
                   ...
                   <span>${localStorage.getItem('watched_' + safeVideo.id) === '1' ? 'Voltooid' : 'Markeer als bekeken'}</span>
                </button>
             </div>
          </div>
        ` : ''}
  ...
  function renderLivros() {
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Uw materialen</h1><p>Klik op de onderstaande collecties om de boeken te bekijken en te downloaden.</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/whi_IMG1_nl.png" alt="Houtsnijden & Houtbewerking-pakket" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>

          <div class="list-container">
            ${featuredHTML || '<p>Er zijn momenteel geen geregistreerde materialen.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderOutrosProdutos() {
      ...
      <div class="premium-book-card">
           ...
           <div class="premium-info">
              ...
              <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                 <i data-lucide="external-link"></i> ${prod.buttonText || 'Lees meer'}
              </a>
           </div>
         </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Andere programma's</h1><p>Ontdek andere programma's en materialen.</p></div></div>
          
          
          <div class="list-container">
            ${prodsHTML || '<p>Binnenkort meer updates!</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Ledensupport</h1><p>Uw tevredenheid is onze prioriteit.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                Kopieer het onderstaande e-mailadres en stuur ons uw vraag. Ons supportteam zal zo snel mogelijk antwoorden.
             </p>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Uw cursussen</h1><p>Leer houtsnijden en houtbewerking stap voor stap met Ethan Blackwood.</p></div></div>
