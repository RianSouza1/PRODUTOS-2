...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "会员区";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "轻松无麸质"}. 保留所有权利。
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>欢迎回来，会员！</h1><p>今天您想访问什么内容？</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">电子书和 PDF 资料</div>
              </div>
            </a>
  ...
  function renderLivros() {
    const booksHTML = APP_DATA.books.map(bk => {
      ...
      <div class="premium-book-card" style="margin-bottom: 2rem;">
           <div class="premium-badge-wrapper">
              <span class="premium-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || '特级'}</span>
              <span class="premium-format">PDF · 立即阅读</span>
           </div>
           ...
           <div class="premium-info" style="padding: 0;">
              ...
              <div style="display: flex; flex-direction: column; gap: 0.75rem; width: 100%; margin-top: 1.5rem;">
                 ...
                 <a href="${bk.downloadUrl}" download class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: transparent; color: var(--text-dark); border: 1px solid var(--border-light);">
                    <i data-lucide="download"></i> 下载 PDF
                 </a>
              </div>
           </div>
        </div >
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel" style="margin-bottom: 1.5rem;"><div class="hero-text"><h1>您的学习资料</h1><p>在下方获取您的指南和赠品。</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/gf_IMG1_zh.png" alt="轻松无麸质系列包" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>
          
          <div class="list-container">
            ${booksHTML || '<p>目前尚未注册任何资料。</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderOutrosProdutos() {
    const prodsHTML = APP_DATA.otherProducts.map(prod => {
      ...
      <div class="premium-book-card">
           <div class="premium-badge-wrapper">
              <span class="premium-badge" style="background-color: ${prod.badgeColor || 'var(--primary)'}">${prod.badgeText || '特级'}</span>
           ...
           <div class="premium-info">
              ...
              <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                 <i data-lucide="external-link"></i> ${prod.buttonText || '阅读更多'}
              </a>
           </div>
        </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>其他项目</h1><p>探索改善您健康的资料。</p></div></div>
          
          
          <div class="list-container">
            ${prodsHTML || '<p>更多内容即将推出！</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>会员支持</h1><p>您的满意是我们的第一宗旨。</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                复制下方邮箱地址并发送带有您问题的邮件。我们的团队会尽快回复。
             </p>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>您的课程</h1><p>随时了解您的在线练习进度</p></div></div>
      ...
      <div class="card-bloco play-item glass-panel ${isPlaying ? 'active-play' : ''}" style="margin-bottom:16px; display:flex; flex-direction:column; padding:0; overflow:hidden;" data-video-id="${vid.id}">
            ...
            <a href="javascript:void(0)" class="play-item-header" style="display:flex; padding: 16px; text-decoration:none; color:inherit;">
              <div style="display:flex; flex-direction:column; justify-content:center; flex:1">
                 ...
                 <p style="margin:0; font-size:0.9rem; color:${isPlaying ? 'var(--text-dark)' : 'var(--text-muted)'}">${vid.duration || '完整课时'}</p>
              </div>
              ${isPlaying
