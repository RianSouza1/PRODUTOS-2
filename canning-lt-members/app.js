...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Namų Produktų Konservavimas"}. Visos teisės saugomos.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Sveiki atvykę, gerbiamas nary!</h1><p>Ką šiandien norėtumėte pasiekti?</p></div></div>
  ...
  function renderLivros() {
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel" style="margin-bottom: 1.5rem;"><div class="hero-text"><h1>Jūsų medžiaga</h1><p>Pasiekite savo vadovus ir premijas žemiau.</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/can_IMG1_lt.png" alt="Namų Produktų Konservavimo komplektas" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>
          
          <div class="list-container">
            ${booksHTML || '<p>Šiuo metu nėra užregistruotos medžiagos.</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Kitos programos</h1><p>Atraskite medžiagą savo sveikatai pagerinti.</p></div></div>
          
          
          <div class="list-container">
            ${prodsHTML || '<p>Netrukus bus daugiau naujienų!</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Narių palaikymas</h1><p>Jūsų pasitenkinimas yra mūsų prioritetas.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                Nukopijuokite žemiau esantį adresą ir išsiųskite el. laišką su savo klausimu. Mūsų komanda atsakys kuo greičiau.
             </p>
      ...
      <div class="card-bloco play-item glass-panel ${isPlaying ? 'active-play' : ''}" style="margin-bottom:16px; display:flex; flex-direction:column; padding:0; overflow:hidden;" data-video-id="${vid.id}">
            ...
            <a href="javascript:void(0)" class="play-item-header" style="display:flex; padding: 16px; text-decoration:none; color:inherit;">
              <div style="display:flex; flex-direction:column; justify-content:center; flex:1">
                 ...
                 <p style="margin:0; font-size:0.9rem; color:${isPlaying ? 'var(--text-dark)' : 'var(--text-muted)'}">${vid.duration || 'Visa pamoka'}</p>
              </div>
              ${isPlaying
