 ...
 * Den Komplette Pianosamlingen — Norsk
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleFsUpdate() {
      ...
      document.querySelectorAll('.video-custom-controls').forEach(bar => {
          ...
          if (btn) {
              btn.innerHTML = isFs 
                  ? `<i data-lucide="minimize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Minimer</span>`
                  : `<i data-lucide="maximize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Fullskjerm</span>`;
          }
      });
      renderIcons();
  }
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Den Komplette Pianosamlingen";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#cbd5e1; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Den Komplette Pianosamlingen"}. Alle rettigheter forbeholdt.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Velkommen, kjære medlem!</h1><p>Hvilke materialer vil du åpne i dag?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Bøker og praktiske veiledninger i PDF</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Kundestøtte</div>
                 <div class="home-block-subtitle">Hjelp og kundeservice</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    const featuredHTML = featuredBooks.map(bk => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'SPESIAL'}</span>
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">Flere materialer</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Materiale'}</span>
            </div>
    ...
    const heroCoverImg = (APP_DATA.books && APP_DATA.books[0] && APP_DATA.books[0].coverImage) ? APP_DATA.books[0].coverImage : "assets/covers/piano_IMG1_no.png";
    const heroCoverAlt = (APP_DATA.config && APP_DATA.config.brandName) ? APP_DATA.config.brandName : "Den Komplette Pianosamlingen";

    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Dine materialer</h1><p>Klikk på samlingene nedenfor for å lese og laste ned bøkene dine.</p></div></div>
          ...
          <div class="list-container">
            ${featuredHTML || '<p>Ingen materialer er registrert for øyeblikket.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderOutrosProdutos() {
    const prodsHTML = APP_DATA.otherProducts.map(prod => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${prod.badgeColor || 'var(--primary)'}">${prod.badgeText || 'SPESIAL'}</span>
               <span class="premium-format">Nettbasert tilgang</span>
            </div>
            ...
            <div class="premium-info">
               ...
               <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                  <i data-lucide="external-link"></i> ${prod.buttonText || 'Mer informasjon'}
               </a>
            </div>
         </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Flere programmer</h1><p>Oppdag flere anbefalte veiledninger og kurs.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Ingen andre produkter er tilgjengelige for øyeblikket.</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Kundestøtte</h1><p>Din tilfredshet er vår høyeste prioritet.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Send oss en melding</h3>
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Kopier e-postadressen nedenfor og send oss ditt spørsmål. Vårt supportteam svarer deg så raskt som mulig.
             </p>
