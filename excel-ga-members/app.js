...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Limistéar na mBall";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 RSCA Library. Gach ceart ar cosaint.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Fáilte, a bhall dhílis!</h1><p>Cad ar mhaith leat rochtain a fháil air inniu?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                  <div class="home-block-title">Leabhair</div>
                  <div class="home-block-subtitle">Leabhair agus Ábhair PDF</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                  <div class="home-block-title">Teagmháil</div>
                  <div class="home-block-subtitle">Cabhair & Tacaíocht</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    const featuredHTML = featuredBooks.map(bk => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
                <span class="premium-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'SPEISIALTA'}</span>
                <span class="premium-format">PDF • Doiciméad in-íoslódáilte</span>
            </div>
              ...
              ${featuresHTML}
               ...
               <div style="display: flex; flex-direction: column; gap: 0.75rem; width: 100%; margin-top: 1.5rem;">
                   <a href="${bk.downloadUrl}" target="_blank" class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: var(--primary); color: #FFF;">
                      <i data-lucide="book-open"></i> Léigh anois
                   </a>
                   <a href="${bk.downloadUrl}" download class="premium-btn" style="width: 100%; text-align: center; justify-content: center; background: transparent; color: var(--text-dark); border: 1px solid var(--border-light);">
                      <i data-lucide="download"></i> Íoslódáil PDF
                   </a>
               </div>
           </div>
        </div>
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">Foinsí breise</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div class="compact-book-info">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Foinse'}</span>
            </div>
            <div class="compact-book-actions">
              <a href="${bk.downloadUrl}" target="_blank" class="compact-action-btn btn-read" title="Léigh anois">
                <i data-lucide="book-open"></i>
              </a>
              <a href="${bk.downloadUrl}" download class="compact-action-btn btn-download" title="Íoslódáil PDF">
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Do chuid ábhar</h1><p>Cliceáil ar na bailiúcháin thíos chun na leabhair a fheiceáil agus a íoslódáil.</p></div></div>
          ...
          <div class="premium-hero-cover-container" style="text-align: center; margin-bottom: 2.5rem; padding: 1.5rem; background: var(--bg-card); border-radius: 16px; border: 1px solid var(--border-light); box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 480px; margin-left: auto; margin-right: auto;">
              <img src="assets/covers/excel_IMG1_ir.png" alt="Pacáiste Excel Mastery" style="max-width: 260px; width: 100%; height: auto; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
          </div>

          <div class="list-container">
             ${featuredHTML || '<p>Níl aon ábhair cláraithe faoi láthair.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderOutrosProdutos() {
    const prodsHTML = APP_DATA.otherProducts.map(prod => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
                <span class="premium-badge" style="background-color: ${prod.badgeColor || 'var(--primary)'}">${prod.badgeText || 'SPEISIALTA'}</span>
                <span class="premium-format">Rochtain ar líne</span>
            </div>
           ...
           <div class="premium-info">
              ...
              <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                  <i data-lucide="external-link"></i> ${prod.buttonText || 'Léigh tuilleadh'}
              </a>
           </div>
         </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Cláir eile</h1><p>Faigh amach faoi chláir agus ábhair eile.</p></div></div>
          
          
          <div class="list-container">
            ${prodsHTML || '<p>Tuilleadh nuashonruithe go luath!</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Tacaíocht do bhaill</h1><p>Is é do shástacht ár dtosaíocht.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Seol teachtaireacht</h3>
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                Cóipeáil an seoladh ríomhphoist thíos agus seol do cheist chugainn. Freagróidh ár bhfoireann tacaíochta chomh luath agus is féidir.
             </p>
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Do chúrsaí</h1><p>Máistreacht ar Excel céim ar chéim lenár gceachtanna físe eisiacha.</p></div></div>
        ...
        <div style="background-color: rgba(16, 185, 129, 0.1); color: var(--primary); border: 1px solid var(--border-light); padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; display: flex; align-items: center; gap: 10px; font-weight: 500; font-size: 0.95rem;" class="glass-panel">
           ...
           <span>Ceachtanna nua go luath</span>
        </div>
            ...
            <a href="javascript:void(0)" class="play-item-header" style="display:flex; padding: 16px; text-decoration:none; color:inherit;">
              <div style="display:flex; flex-direction:column; justify-content:center; flex:1">
                 <h4 style="margin:0 0 4px; font-size:1.1rem; color:${isPlaying ? 'var(--primary)' : 'var(--text-dark)'}">${vid.title}</h4>
                  <p style="margin:0; font-size:0.9rem; color:${isPlaying ? 'var(--text-dark)' : 'var(--text-muted)'}">${vid.duration || 'Cúrsa iomlán'}</p>
              </div>
              ${isPlaying
