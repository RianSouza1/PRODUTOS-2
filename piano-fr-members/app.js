/**
 * APP CORE ENGINE
 * ESPACE MEMBRES (Mobile First & Senior Friendly)
 * La Collection Complète de Piano — Français
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleFsUpdate() {
      ...
      document.querySelectorAll('.video-custom-controls').forEach(bar => {
          ...
          if (btn) {
              btn.innerHTML = isFs 
                  ? `<i data-lucide="minimize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Minimiser</span>`
                  : `<i data-lucide="maximize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Plein écran</span>`;
          }
      });
      renderIcons();
  }
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "La Collection Complète de Piano";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#cbd5e1; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "La Collection Complète de Piano"}. Tous droits réservés.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Bienvenue, cher membre !</h1><p>Quelles ressources souhaitez-vous consulter aujourd'hui ?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Livres et guides pratiques au format PDF</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Support Client</div>
                 <div class="home-block-subtitle">Aide et assistance personnalisée</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">Autres ressources</h2>
    ...
    const heroCoverImg = (APP_DATA.books && APP_DATA.books[0] && APP_DATA.books[0].coverImage) ? APP_DATA.books[0].coverImage : "assets/covers/piano_IMG1_fr.png";
    const heroCoverAlt = (APP_DATA.config && APP_DATA.config.brandName) ? APP_DATA.config.brandName : "La Collection Complète de Piano";

    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Vos ressources</h1><p>Consultez et téléchargez vos livres de piano ci-dessous.</p></div></div>
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Autres programmes</h1><p>Découvrez d'autres méthodes et formations recommandées.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Aucun autre produit disponible pour le moment.</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Support Client</h1><p>Votre satisfaction est notre priorité absolue.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Copiez l'adresse e-mail ci-dessous et écrivez-nous pour toute question. Notre équipe vous répondra dans les plus brefs délais.
             </p>
