...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function mountMailTo() {
    const { contactEmail, emailSubject, Привет, команда поддержки! Мне нужна помощь с моим разделом для участников пчеловодства.%0A%0AМеня зовут: ______. } = APP_DATA.config;
    return `mailto:${contactEmail}?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(Привет, команда поддержки! Мне нужна помощь с моим разделом для участников пчеловодства.%0A%0AМеня зовут: ______.)}`;
  }
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Добро пожаловать, участник</h1><p>К чему бы вы хотели получить доступ сегодня?</p></div></div>
  ...
  // TELA: КНИГИ (Materiais tipo Bundle)
  function renderLivros() {
    const booksHTML = APP_DATA.books.map(bk => {
      ...
      <div class="premium-book-card">
           <div class="premium-badge-wrapper">
              ...
              <span class="premium-format">PDF • Цифровая загрузка</span>
           </div>
    ...
    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Ваши материалы</h1><p>Нажмите на коллекцию ниже, чтобы просмотреть ее содержимое.</p></div></div>
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Больше программ</h1><p>Откройте для себя дополнительные материалы.</p></div></div>
  ...
  function renderContato() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Поддерживать</h1><p>Ваше удовлетворение является нашим приоритетом.</p></div></div>
