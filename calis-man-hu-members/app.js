/**
 * APP CORE ENGINE
 * TAGI FELÜLET (Mobile First & Senior Friendly)
 * Katonai Kaliszténika Férfiaknak — Magyar
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "Katonai Kaliszténika Férfiaknak";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#6B7280; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "Katonai Kaliszténika Férfiaknak"}. Minden jog fenntartva.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Üdvözöljük, kedves Tagunk!</h1><p>Melyik tananyagot szeretnéd ma megnyitni?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Könyvek és letölthető PDF anyagok</div>
              </div>
            </a>
            ...
            <a href="#videos" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Oktatóvideók és technika</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Ügyfélszolgálat</div>
                 <div class="home-block-subtitle">Segítség és ügyféltámogatás</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    const featuredHTML = featuredBooks.map(bk => {
      ...
      <div class="premium-book-card">
            <div class="premium-badge-wrapper">
               <span class="premium-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'SPECIÁLIS'}</span>
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">További tananyagok</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Tananyag'}</span>
            </div>
    ...
    const heroCoverImg = (APP_DATA.books && APP_DATA.books[0] && APP_DATA.books[0].coverImage) ? APP_DATA.books[0].coverImage : "assets/covers/man_IMG1_hu.png";
    const heroCoverAlt = (APP_DATA.config && APP_DATA.config.brandName) ? APP_DATA.config.brandName : "Katonai Kaliszténika Férfiaknak";

    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Tananyagaid</h1><p>Kattints az alábbi kötetekre az olvasáshoz és a letöltéshez.</p></div></div>
          ...
          <div class="list-container">
            ${featuredHTML || '<p>Jelenleg nincsenek elérhető tananyagok.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderVideos() {
    ...
    rootEl.innerHTML = `
      ...
      <div class="playlist-container" style="padding: 24px var(--safe-padding);">
        <div class="hero-card glass-panel" style="margin-top:-24px;"><div class="hero-text"><h1>Oktatóvideók</h1><p>Katonai saját testsúlyos gyakorlatok és helyes technika 40+ férfiaknak.</p></div></div>
        ...
        <div style="background-color: rgba(16, 185, 129, 0.1); color: var(--primary); border: 1px solid var(--border-light); padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; display: flex; align-items: center; gap: 10px; font-weight: 500; font-size: 0.95rem;" class="glass-panel">
           ...
           <span>Kattints a gyakorlatra az oktatóvideó elindításához</span>
        </div>
      ...
      ${isPlaying ? `
              <div class="play-item-body" style="padding: 0 16px 16px 16px; animation: slideDown 0.3s ease;">
                 ...
                 <div style="margin-top: 10px; padding: 10px 14px; background: var(--bg-card); border-radius: 8px; border: 1px solid var(--border-light); font-size: 0.88rem; color: var(--text-muted); display: flex; align-items: center; gap: 8px;">
                     ...
                     <span><strong>Útmutatás:</strong> ${vid.obs}</span>
                 </div>
                 ` : ''}
              </div>
            ` : ''}
  ...
  function renderOutrosProdutos() {
      ...
      <div class="premium-book-card">
            ...
            <div class="premium-info">
               ...
               <a href="${prod.linkUrl}" target="_blank" class="premium-btn">
                  <i data-lucide="external-link"></i> ${prod.buttonText || 'További információ'}
               </a>
            </div>
         </div>
      `;
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>További programok</h1><p>Fedezz fel további ajánlott útmutatókat és tanfolyamokat.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Jelenleg nincsenek elérhető további termékek.</p>'}
          </div>
        </div>
      `;
  }
  ...
  function renderContato() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Ügyfélszolgálat</h1><p>Az Ön elégedettsége a legfontosabb számunkra.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Küldjön nekünk üzenetet</h3>
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Másolja ki az alábbi e-mail címet, és küldje el kérdését. Ügyfélszolgálatunk a lehető leghamarabb válaszol Önnek.
             </p>
