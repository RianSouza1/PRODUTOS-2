 ...
 * A Teljes Zongoragyűjtemény — Magyar
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleFsUpdate() {
      ...
      document.querySelectorAll('.video-custom-controls').forEach(bar => {
          ...
          if (btn) {
              btn.innerHTML = isFs 
                  ? `<i data-lucide="minimize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Kicsinyítés</span>`
                  : `<i data-lucide="maximize" style="width: 22px; height: 22px; color: #ffffff;"></i><span>Teljes képernyő</span>`;
          }
      });
      renderIcons();
  }
  ...
  function initGlobalConfig() {
    if (APP_DATA.config) {
      brandTitle.innerText = APP_DATA.config.brandName || "A Teljes Zongoragyűjtemény";
    }
  }
  ...
  function handleRouting() {
    ...
    rootEl.insertAdjacentHTML('beforeend', `
       <footer class="app-footer" style="text-align:center; font-size:0.75rem; font-weight: 500; color:#cbd5e1; padding: 2rem 1rem 1.5rem; letter-spacing: 0.5px;">
         &copy; 2026 ${APP_DATA.config.brandName || "A Teljes Zongoragyűjtemény"}. Minden jog fenntartva.
       </footer>
    `);
  ...
  function renderHome() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Üdvözöljük, kedves tagunk!</h1><p>Milyen anyagokat szeretnél ma megnyitni?</p></div></div>
          ...
          <div class="home-grid">
            ...
            <a href="#livros" class="home-block glass-panel">
              ...
              <div>
                 ...
                 <div class="home-block-subtitle">Könyvek és gyakorlati útmutatók PDF-ben</div>
              </div>
            </a>
            ...
            <a href="#contato" class="home-block glass-panel">
              ...
              <div>
                 <div class="home-block-title">Támogatás</div>
                 <div class="home-block-subtitle">Segítség és ügyfélszolgálat</div>
              </div>
            </a>
  ...
  function renderLivros() {
    ...
    if (compactBooks.length > 0) {
      compactHTML = `
      <h2 class="section-divider-title">További anyagok</h2>
      <div class="compact-book-list">
        ${compactBooks.map(bk => `
          <div class="compact-book-card">
            <div style="display: flex; align-items: center; gap: 12px; flex: 1; min-width: 0;">
              <h4 class="compact-book-title">${bk.title}</h4>
              <span class="compact-book-badge" style="background-color: ${bk.badgeColor || 'var(--primary)'}">${bk.badgeText || 'Anyag'}</span>
            </div>
    ...
    const heroCoverImg = (APP_DATA.books && APP_DATA.books[0] && APP_DATA.books[0].coverImage) ? APP_DATA.books[0].coverImage : "assets/covers/piano_IMG1_hu.png";
    const heroCoverAlt = (APP_DATA.config && APP_DATA.config.brandName) ? APP_DATA.config.brandName : "A Teljes Zongoragyűjtemény";

    rootEl.innerHTML = `
      <div class="page-view" style="padding-bottom: 0;">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Anyagaid</h1><p>Kattints az alábbi gyűjteményekre a könyveid olvasásához és letöltéséhez.</p></div></div>
          ...
          <div class="list-container">
            ${featuredHTML || '<p>Jelenleg nincsenek elérhető anyagok.</p>'}
            ${compactHTML}
          </div>
        </div>
      `;
  ...
  function renderOutrosProdutos() {
    ...
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>További programok</h1><p>Fedezz fel további ajánlott útmutatókat és képzéseket.</p></div></div>
          
          <div class="list-container">
            ${prodsHTML || '<p>Jelenleg nincsenek egyéb termékek.</p>'}
          </div>
        </div>
      `;
  }

  // NÉZET: TÁMOGATÁS / KAPCSOLAT
  function renderContato() {
    rootEl.innerHTML = `
      <div class="page-view">
          <div class="hero-card glass-panel"><div class="hero-text"><h1>Ügyfélszolgálat</h1><p>Az elégedettséged a legfőbb prioritásunk.</p></div></div>
          ...
          <div class="card-bloco glass-panel" style="text-align: center; padding: 2.5rem 1.5rem;">
             ...
             <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color:var(--text-dark)">Küldj nekünk üzenetet</h3>
             <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 2rem; line-height:1.5;">
                 Másold ki az alábbi e-mail címet, és küldd el kérdésedet. Támogató csapatunk a lehető leghamarabb válaszol neked.
             </p>
